QuickTime = {
	...
	{		
		...
		if (!QuickTime.installed && !QuickTime.upgrade) return;		
		
		var regexp = new RegExp("\\.m4v(\\?|$)");
		...
		{			
			...
			if (QuickTime.installed)
			{
				...
				// edited by Jack divs[i].style.height = divs[i].clientHeight + 16 + "px";
				divs[i].style.height = divs[i].clientHeight;
			}
		}
